...
class Taylor(ApproximationStrategy):
  ...
  def apply(self, h: float, x: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]: # type: ignore
    # aplica la función de aproximación
    homx = augmented_x(x)
    Phi = self.get_matrix(h)
    return Phi @ homx
  ...
  def _get_M(self, normA: float) -> float:
    ...
    for x in self._S:
      ...
      norm = float(np.linalg.norm(self._A @ x + self._b, ord=2))
      if norm > m:
        m = norm
    return 1/normA * m
  ...
  def error_sequence(self, h: float, K: int) -> list[float]:
    ...
    normHomA = float(np.linalg.norm(self._homA, ord=2))
  ...
  def get_matrix(self, h: float) -> npt.NDArray[np.float64]: # type: ignore
